...
describe('Select', () => {
    ...
    it('should work correctly when value is 0', async () => {
        const PRIORITY = ['Zero', 'One', 'Two', 'Three', 'Four', 'Five'];
        const value = 1;
        const { getByRole, getAllByText } = render(
            <Select
                ...
                value={value}
                valueToString={(value) => PRIORITY[value]}
            />
        );
        const select = getByRole('listbox');
        ...
        const valueString = getAllByText(PRIORITY[value]);
        expect(valueString.length).toBe(2);
    });
});
