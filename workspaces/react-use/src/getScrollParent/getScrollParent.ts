...
const scroll = (pNode) => /(auto|scroll)/.test(overflow(pNode));

export const getScrollParent = (node, getAll?: boolean): any => {
...
export const getScrollParents = (node): any[] => getScrollParent(node, true);

export const isElementInViewport = (element, viewport): boolean => {
