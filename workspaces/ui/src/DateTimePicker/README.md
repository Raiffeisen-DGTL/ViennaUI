...
Границы выбора времени определяют нижнее и верхнее значения при выборе времени из выпадающего списка компонента `Select`.
По умолчанию `timeFrom='00:00` и `timeTo='23:30`
...
Шаг выбора времени определяет значения при выборе времени из выпадающего списка компонента `Select`.
По умолчанию `step={30}`
...
Для того, чтобы отобразить компонент, как неактивный, необходимо установить свойство `disabled` тому дочернему элементу: 
**календарю или полю для выбора/ввода даты, или им обоим**, выбор значения в котором должнет быть заблокирован.
...
Для того, чтобы отобразить компонент, как невалидный, необходимо установить свойство `disabled` тому дочернему элементу: 
**календарю или полю для выбора/ввода даты, или им обоим**, значение в котором выбрано некорректно.
